...
| 工具/技能/权限边界 | `createToolRegistry()`、`discoverSkills()` | `gemini-cli/packages/core/src/config/config.ts:3257`, `1367-1379`, `2742-2749` | `setTools()` 和 `Scheduler._processToolCall()` 只能在启动确定的能力面内运行 |
...
| `sendMessageStream() → processTurn()` | `gemini-cli/packages/core/src/core/client.ts:868-940` | `925-933` | reset turn 状态、运行 agent hook |
| `processTurn() → Turn.run()` | `gemini-cli/packages/core/src/core/client.ts:585-865` | `726-734` | 上下文压缩、token 检查、模型路由、工具声明刷新 |
| `Turn.run() → GeminiChat.sendMessageStream()` | `gemini-cli/packages/core/src/core/turn.ts:253-404` | `263-270` | 进入底层模型流封装层 |
...
1. `Turn.run()` — `gemini-cli/packages/core/src/core/turn.ts:325-331`
...
1. `useToolScheduler.schedule()` — `gemini-cli/packages/cli/src/ui/hooks/useToolScheduler.ts:176-190`，调用点 `182`
   调 `scheduler.schedule(request, signal)`；完成后调 `onCompleteRef.current(results)`。
2. `Scheduler.schedule()` — `gemini-cli/packages/core/src/scheduler/scheduler.ts:191-217`
   决定直接 `_startBatch()` 或进入 `_enqueueRequest()`。
3. `Scheduler._startBatch()` — `gemini-cli/packages/core/src/scheduler/scheduler.ts:297-339`，调用点 `325-329` → `333`
   验证工具调用，进入 `_processQueue(signal)`。
4. `Scheduler._processNextItem()` — `gemini-cli/packages/core/src/scheduler/scheduler.ts:416-527`，调用点 `463-485`
...
| 进入 Scheduled 状态 | — | `gemini-cli/packages/core/src/scheduler/scheduler.ts:683` |

### 5.5 工具执行：`Scheduler._execute()` → `ToolExecutor`

1. `Scheduler._execute()` — `gemini-cli/packages/core/src/scheduler/scheduler.ts:691-860`，调用点 `717-734`
...
`GeminiClient.processTurn()` — `gemini-cli/packages/core/src/core/client.ts:672`
...
`GeminiClient.processTurn()` — 调用点 `gemini-cli/packages/core/src/core/client.ts:740-755`
...
`Turn.callCounter`（`gemini-cli/packages/core/src/core/turn.ts:239`）只是为缺失 id 的 function call 生成 fallback `callId`，**不负责任何 loop detection**。
...
- **`createToolRegistry()` 行号漂移风险**：当前位于 `gemini-cli/packages/core/src/config/config.ts:3257`，但 `config.ts` 文件体量大（超 3500 行），后续改动容易使行号大幅偏移，建议拆文件。

---

## 8.1 Scheduler 三阶段处理详解

`gemini-cli/packages/core/src/scheduler/scheduler.ts:405-504` 的 `_processQueue()` 使用 while 循环 + 三阶段模式处理工具调用：
...
Gemini CLI 独特的并发决策机制——由模型决定工具是否串行（`gemini-cli/packages/core/src/scheduler/scheduler.ts:549-558`）：
...
`_recoverFromLoop()`（`gemini-cli/packages/core/src/core/client.ts:778-797`）注入一条用户消息告知模型其行为在重复，然后重新进入 `processTurn()`。这是四个工程中唯一实现了基于 LLM 分析的主动循环模式检测的方案。
