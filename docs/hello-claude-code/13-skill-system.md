...
源码上，`claude-code/src/commands.ts:353` 聚合 skill 来源，`claude-code/src/commands.ts:360` 同时加载 skill dir 与 plugin skill，`claude-code/src/commands.ts:451` 把 skill、plugin、workflow 等命令源合并，`claude-code/src/commands.ts:479` 到 `claude-code/src/commands.ts:509` 处理动态 skill 去重和插入位置。运行时还会在 `claude-code/src/bootstrap/state.ts:1501` 到 `claude-code/src/bootstrap/state.ts:1558` 记录 invoked skills，以便 compaction 后保留已激活 skill 的上下文。
