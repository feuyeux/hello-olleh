...
| --- | --- | --- | --- |
| 用户输入预处理 | `claude-code/src/QueryEngine.ts:419`, `claude-code/src/utils/processUserInput/processUserInput.ts:85` | 普通 user message、slash command 展开文本、附件或结构化输入 | 把 UI/SDK 输入转成 query 可消费的消息；prompt command 在这里变成文本任务 |
| 主系统提示构建 | `claude-code/src/constants/prompts.ts:444`, `claude-code/src/constants/prompts.ts:573` | system prompt sections | 定义 Claude Code 身份、任务方式、工具使用规则，并用 dynamic boundary 支撑 cache 分层 |
| 用户/系统上下文读取 | `claude-code/src/context.ts:116`, `claude-code/src/context.ts:155` | `userContext` 与 `systemContext` | `CLAUDE.md`、日期、git status、cache breaker 等上下文在这里生成 |
| 三件套聚合 | `claude-code/src/utils/queryContext.ts:44`, `claude-code/src/QueryEngine.ts:295` | `defaultSystemPrompt + userContext + systemContext` | QueryEngine 在请求开始前拿到可缓存前缀所需的三类上下文 |
| 历史裁剪边界 | `claude-code/src/query.ts:365` | messages after compact boundary | 只把 compact boundary 之后的消息进入本轮请求，避免旧历史无限膨胀 |
| system context 追加 | `claude-code/src/query.ts:449`, `claude-code/src/utils/api.ts:437` | system prompt 尾部动态块 | git status 等高变化状态不进入静态主前缀，而是请求前附加 |
| `CLAUDE.md` 注入 | `claude-code/src/query.ts:659`, `claude-code/src/utils/api.ts:449` | meta user message | 项目指令以 user-context 形式压到消息前面，不是主 system prompt 本体 |
| 工具池组装 | `claude-code/src/tools.ts:271`, `claude-code/src/tools.ts:345` | tool list | 内建工具与 MCP 工具合并、按权限 deny 规则预过滤，并为 cache 稳定排序 |
| 工具 schema/prompt 序列化 | `claude-code/src/utils/api.ts:119`, `claude-code/src/services/api/claude.ts:1238` | API `tools` 字段 | 每个工具的 `prompt()`、input schema、MCP/defer 元数据被转成模型可见工具声明 |
| API system blocks | `claude-code/src/services/api/claude.ts:1377`, `claude-code/src/utils/api.ts:3214` | Anthropic system text blocks | 在发送前加 cache_control，并应用 system prompt prefix splitting |
| 最终模型请求 | `claude-code/src/services/api/claude.ts:1702`, `claude-code/src/services/api/claude.ts:1711`, `claude-code/src/services/api/claude.ts:1823` | `messages + system + tools` | 真正进入 Anthropic Messages API 的闭合点 |
| tool_use 回注 | `claude-code/src/query.ts:832`, `claude-code/src/services/tools/toolExecution.ts:400`, `claude-code/src/query.ts:1065` | tool_result user message + follow-up loop | 模型发 tool_use 后，runtime 执行工具并把 tool_result 写回下一次采样上下文 |
| compact 降级 | `claude-code/src/commands/compact/compact.ts:98`, `claude-code/src/services/compact/compact.ts:389`, `claude-code/src/services/compact/compact.ts:618` | compact summary user message | 长上下文或手动 `/compact` 时，将旧消息摘要化后作为继续会话的上下文 |
...
| 函数/类型 | 文件 | 职责 |
|----------|------|------|
| `getSystemPrompt()` | `claude-code/src/constants/prompts.ts:444` | 构造主 system prompt section 数组，并插入 dynamic boundary / memory / feature sections |
| `fetchSystemPromptParts()` | `claude-code/src/utils/queryContext.ts:44` | 并行获取 `defaultSystemPrompt`、`userContext`、`systemContext`，是请求前 prompt 三件套入口 |
| `getUserContext()` | `claude-code/src/context.ts:155` | 读取 `CLAUDE.md` 与当前日期，供 `prependUserContext()` 注入 |
| `getSystemContext()` | `claude-code/src/context.ts:116` | 读取 git status / cache breaker 等动态系统上下文 |
| `appendSystemContext()` | `claude-code/src/utils/api.ts:437` | 将 system context 追加到 system prompt 尾部 |
| `prependUserContext()` | `claude-code/src/utils/api.ts:449` | 将 `CLAUDE.md`/日期等 user context 包成 meta user message 并放到消息前面 |
| `queryLoop()` | `claude-code/src/query.ts:241` | 主 agent loop：裁剪历史、拼装 prompt、调用模型、处理 tool_use 和 follow-up |
| `toolToAPISchema()` | `claude-code/src/utils/api.ts:119` | 将 Tool 的 prompt/schema/MCP 元数据转成 Anthropic API tool schema |
| `buildSystemPromptBlocks()` | `claude-code/src/utils/api.ts:3214` | 将 system prompt 分块并加 `cache_control` |
| `queryModel()` | `claude-code/src/services/api/claude.ts:1018` | 最终请求构造与 Anthropic Messages API streaming 调用 |
| `compactConversation()` | `claude-code/src/services/compact/compact.ts:389` | 用 compact prompt 摘要旧历史并生成 post-compact message set |
