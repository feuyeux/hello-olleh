...
| --- | --- | --- | --- | --- |
| schema timeout | `claude-code/src/schemas/hooks.ts:42`, `claude-code/src/schemas/hooks.ts:75`, `claude-code/src/schemas/hooks.ts:101`, `claude-code/src/schemas/hooks.ts:144` | command、prompt、http、agent hook 都有 timeout 字段 | 超时按 hook 执行错误处理 | 配置层只定义，不直接写 transcript |
| once/background | `claude-code/src/schemas/hooks.ts:51`, `claude-code/src/schemas/hooks.ts:58` | hook 可设置一次性或后台执行 | 后台 hook 不阻塞主链路 | 后台错误通过通知/日志暴露 |
| Stop hook | `claude-code/src/query/stopHooks.ts:269` | turn 结束前检查 `preventContinuation` | 可阻止继续并返回 `hook_stopped_continuation` | `stopHooks.ts:310` 会给 verbose/transcript 模式提示错误 |
| TaskCompleted / TeammateIdle | `claude-code/src/query/stopHooks.ts:383`, `claude-code/src/query/stopHooks.ts:425` | stop hook 之后处理 teammate/task 场景 | 与 Stop hook 一样支持阻止继续 | 通过 hook progress/response 进入可观测面 |
| query 主循环消费 | `claude-code/src/query.ts:1281`, `claude-code/src/query.ts:1393` | query 根据 hook result 决定 stop/continue | API error 路径跳过 stop hook，避免死循环 | hook continuation 作为 attachment 被模型/日志看见 |
