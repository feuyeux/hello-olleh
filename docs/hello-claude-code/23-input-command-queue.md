...
| --- | --- | --- | --- | --- |
| 自然语言 prompt | 是，进入 `query()` | 否 | 通常否 | `claude-code/src/query.ts:187` |
| prompt slash command | 取决于 command 返回 | 是，走 `processPromptSlashCommand()` | 可能，skill command 可改权限/工具上下文 | `claude-code/src/utils/processUserInput/processSlashCommand.tsx:817` |
...
| interrupt/cancel | 否 | 否 | 不改变工具池，但会终止当前运行态 | `claude-code/src/query.ts:1489` |
