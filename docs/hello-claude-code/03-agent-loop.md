...
`claude-code/src/query.ts` 里的 `query()` 不是“调用一次模型 API”的薄包装，而是整个系统的对话执行状态机。
...
- `claude-code/src/query.ts:219-238`
- `claude-code/src/query.ts:241-279`
...
关键代码：`claude-code/src/query.ts:263-279`
...
关键代码：`claude-code/src/query.ts:323-460`

## 5.1 发 `stream_request_start`

`claude-code/src/query.ts:337`

这是给上层 UI/SDK 的控制事件，表示一次新的 API 请求即将开始。

## 5.2 生成 query chain tracking

`claude-code/src/query.ts:346-363`
...
从 `claude-code/src/query.ts:365-468` 可以确认，模型请求前会先经过一条上下文治理流水线：
...
- `claude-code/src/query.ts:449-450`
- `claude-code/src/utils/queryContext.ts:44-73`
...
关键代码：`claude-code/src/query.ts:551-580`
...
关键文件：`claude-code/src/services/api/claude.ts`

重点代码：

- `claude-code/src/services/api/claude.ts:1358-1379`
- `claude-code/src/services/api/claude.ts:1538-1728`
- `claude-code/src/services/api/claude.ts:1777-1833`
...
关键代码：`claude-code/src/services/api/claude.ts:1822-1833`
...
关键代码：`claude-code/src/query.ts:652-864`
...
`claude-code/src/query.ts:829-845`
...
关键代码：`claude-code/src/query.ts:1185-1357`
...
关键代码：`claude-code/src/query.ts:1363-1435`
...
关键代码：`claude-code/src/utils/queryContext.ts:44-73`
...
`StreamingToolExecutor`（`claude-code/src/services/tools/StreamingToolExecutor.ts`）实现了模型流式输出期间的**流水线并行工具执行**。当模型还在流式输出时，已经完成解析的 `tool_use` 块会被立即排入执行队列。

**入队** — `claude-code/src/services/tools/StreamingToolExecutor.ts:83`：
...
**并发门控** — `claude-code/src/services/tools/StreamingToolExecutor.ts:137-143`：
...
**队列处理** — `claude-code/src/services/tools/StreamingToolExecutor.ts:153`：
...
`claude-code/src/services/tools/toolOrchestration.ts:96-119`
...
最大并发度 — `claude-code/src/services/tools/toolOrchestration.ts:9-11`：
...
| 级别 | 函数 | 代码位置 | 触发条件 | 职责 |
|------|------|----------|----------|------|
| History Snip | `snipModule.snipCompactIfNeeded()` | `claude-code/src/query.ts:346-353` | 特性门控 `HISTORY_SNIP` | 裁剪老旧历史，释放 token |
| Microcompact | `deps.microcompact()` | `claude-code/src/query.ts:353-375` | **每轮迭代**都运行 | 轻量级工具结果裁剪 |
| Autocompact | `deps.autocompact()` | `claude-code/src/query.ts:404-545` | token 接近上限 | 主动摘要压缩 |
| Reactive Compact | `reactiveCompact.tryReactiveCompact()` | `claude-code/src/query.ts:1171-1220` | API 返回 `prompt_too_long` | 被动紧急压缩 |

Reactive Compact 是最后一道防线，只在 API 请求因上下文过长而失败后才触发（`claude-code/src/query.ts:1171`）：
...
| 退出路径 | 代码位置 | 条件 | 返回值 |
|----------|----------|------|--------|
| 正常完成 | `claude-code/src/query.ts:1336` | `!needsFollowUp` 且无待恢复错误 | `{ reason: 'completed' }` |
| 达到 maxTurns | `claude-code/src/query.ts:1705-1710` | `nextTurnCount > maxTurns` | `{ reason: 'max_turns' }` |
| 用户中断 | `claude-code/src/query.ts:984-1048` | `abortController.signal.aborted` | `{ reason: 'aborted_streaming' }` |
| Stop hook 阻止 | `claude-code/src/query.ts:1243-1380` | hook 返回 stop 指令 | `{ reason: 'stop_hook' }` |
| Max output tokens 耗尽 | `claude-code/src/query.ts:1206-1260` | 3 次恢复尝试均失败 | 错误上抛 |
| 上下文不可恢复 | `claude-code/src/query.ts:1171-1220` | reactive compact 失败 | 错误上抛 |

**Max Output Tokens 恢复机制**（`claude-code/src/query.ts:1206-1260`）：
...
`claude-code/src/query.ts:735-1060`：当主模型因 demand/rate limit 失败时，可切换到 `fallbackModel` 重试：
...
`claude-code/src/services/api/withRetry.ts:178`：`withRetry()` 处理 429（rate limit）和 5xx 错误的指数退避重试，同时集成 auth refresh 和 fallback 模型切换。
...
| 主题 | 代码锚点 | 说明 |
| --- | --- | --- |
| query 入口 | `claude-code/src/query.ts:219-238` | generator 外层包装 |
| queryLoop 初始 state | `claude-code/src/query.ts:241-279` | 多轮状态机的状态定义 |
| 请求前上下文治理 | `claude-code/src/query.ts:365-468` | budget, snip, microcompact, collapse, autocompact |
| API streaming 调用 | `claude-code/src/query.ts:652-864` | `deps.callModel(...)` 的主循环 |
| max token 恢复与 stop hooks | `claude-code/src/query.ts:1185-1357` | query 结束前的恢复/阻断策略 |
| 工具回流 | `claude-code/src/query.ts:1363-1435` | `tool_use -> tool_result -> 下一轮` |
| 系统提示词拼装 | `claude-code/src/services/api/claude.ts:1358-1379` | system prompt block 的最终构造 |
| 请求参数生成 | `claude-code/src/services/api/claude.ts:1538-1728` | thinking、betas、context_management、output_config |
| 真正发请求 | `claude-code/src/services/api/claude.ts:1777-1833` | raw streaming create + response headers |
...
| 函数 | 文件 | 行号 | 职责 |
|------|------|------|------|
| `query()` | `claude-code/src/query.ts` | — | 单次用户 turn 的状态机入口：上下文治理、模型调用、工具执行 |
| `queryLoop()` | `claude-code/src/query.ts` | 241 | 内层无限循环：每轮推理 + tool_use 批处理 |
| `callModel()` | `claude-code/src/services/api/claude.ts` | — | 发出流式模型请求，yield 流事件 |
| `runTools()` | `claude-code/src/query.ts` | — | 批量工具执行入口 |
| `runToolUse()` | `claude-code/src/query.ts` | — | 单工具执行：权限检查 + 调用 `tool.call()` |
| `getMessagesAfterCompactBoundary()` | `claude-code/src/query.ts` | — | 获取 compact 边界后的有效历史消息 |
| `paramsFromContext()` | `claude-code/src/services/api/claude.ts` | — | 将 ToolUseContext 折叠为 API 请求参数（cache/thinking/betas）|
| `withRetry()` | `claude-code/src/services/api/claude.ts` | — | 重试 + fallback + auth refresh + context overflow 状态机 |
| `asSystemPrompt()` | `claude-code/src/query.ts` | 449 | 组装最终 system prompt |
...
| 证据等级 | 本章用法 |
| --- | --- |
| 源码确认 | `claude-code/src/query.ts`、`claude-code/src/services/api/claude.ts`、`claude-code/src/services/tools/*` 中能直接看到的调用链 |
