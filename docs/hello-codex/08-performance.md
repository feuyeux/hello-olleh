...
源码锚点上，turn submission 可从 `sources/codex/codex-rs/core/src/codex.rs:697` 追到 core，工具治理从 `sources/codex/codex-rs/core/src/tools/orchestrator.rs:111` 看 approval/sandbox policy 注入，TUI 提交路径可从 `sources/codex/codex-rs/tui/src/app.rs:1812` 看，app-server 事件消费压力可对照 `sources/codex/codex-rs/app-server-client/src/lib.rs:731`。这组指标把“快不快”拆成 provider、runtime、tool、UI 四类，而不是只看总响应时间。
