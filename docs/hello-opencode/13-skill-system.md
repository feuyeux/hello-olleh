...
| 维度 | 源码锚点 | 结论 |
| --- | --- | --- |
| 来源 | `opencode/packages/opencode/src/skill/index.ts:143` | `Config.directories()` 返回的目录都会参与 skill 发现 |
| 格式校验 | `opencode/packages/opencode/src/skill/index.ts:84` | frontmatter 至少校验 `name`、`description` |
| 目录记录 | `opencode/packages/opencode/src/skill/index.ts:95` | 命中的 skill 目录会进入 `state.dirs` |
| 缓存 | `opencode/packages/opencode/src/skill/index.ts:191` | `Skill.ensure()` 复用 InstanceState 中的 discovery cache |
| 可用性过滤 | `opencode/packages/opencode/src/skill/index.ts:212` | `Skill.available()` 会按 agent permission 过滤 |
| 权限规则 | `opencode/packages/opencode/src/permission/index.ts:278` | config permission 转成 ruleset，再由 `Permission.evaluate()` 判断 |
| Prompt 注入 | `opencode/packages/opencode/src/skill/index.ts:242` | 可用 skill 被渲染成 “Available Skills” 列表 |
