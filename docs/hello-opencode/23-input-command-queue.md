...
`sources/opencode/packages/opencode/src/session/prompt.ts:1823-1973`：
...
| --- | --- | --- |
| prompt | `sources/opencode/packages/opencode/src/server/routes/session.ts` -> `SessionPrompt.prompt()` | durable user message 编译 |
...
| TUI event | `sources/opencode/packages/opencode/src/server/routes/tui.ts` / Bus | 多表面输入统一投影 |
