...
| 阶段 | 源码锚点 | 事件/状态 |
| --- | --- | --- |
| 订阅入口 | `sources/opencode/packages/opencode/src/server/routes/event.ts:19` | `/event` 暴露 `event.subscribe` |
| SSE stream | `sources/opencode/packages/opencode/src/server/routes/event.ts:35` | 通过 `streamSSE()` 持续输出事件 |
...
| message update | `sources/opencode/packages/opencode/src/session/index.ts:700` | message 更新进入 Bus |
| part update | `sources/opencode/packages/opencode/src/session/index.ts:770` | `message.part.updated` 是 UI/CLI 的主要增量信号 |
| part delta | `sources/opencode/packages/opencode/src/session/index.ts:787` | text/reasoning delta 用轻量事件推送 |
