...
| 函数/类型 | 文件 | 职责 |
|----------|------|------|
| `Config` class | `packages/core/src/config/config.ts` | 组合根兼服务容器：装配所有核心依赖，是运行时唯一真相源 |
| `Config.initialize()` | `config.ts:1289` | 装配入口：创建 ToolRegistry、MCP servers、GeminiClient、SkillManager |
| `Config.getSettings()` | `config.ts` | 读取合并后配置，按 global → project → 环境变量 → CLI flag 优先级返回 |
| `loadDotEnv()` | `packages/core/src/config/` | 加载 `.gemini/.env`，注入环境变量到进程 |
| `Config.getWorkspaceTrustPolicy()` | `config.ts` | 返回当前工作区信任等级，影响工具权限与 sandbox 策略 |
| `Storage.initialize()` | `packages/core/src/config/storage.ts` | 计算全局/项目级持久化根目录，确保目录存在 |
