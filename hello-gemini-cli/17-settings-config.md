...
配置合并的核心在 `gemini-cli/packages/cli/src/config/settings.ts`。
...
结合 `gemini-cli/packages/core/src/config/storage.ts`，当前仓库里最重要的路径如下：
...
`gemini-cli/packages/cli/src/config/settingsSchema.ts` 说明 Gemini CLI 的 settings 已经是 schema 驱动，而不是松散 JSON。
...
真正把这些设置变成可执行系统的是 `gemini-cli/packages/cli/src/config/config.ts`。
...
| 函数/类型 | 文件 | 职责 |
|----------|------|------|
| `Config` class | `gemini-cli/packages/core/src/config/config.ts` | 组合根兼服务容器：装配所有核心依赖，是运行时唯一真相源 |
| `Config.initialize()` | `gemini-cli/packages/core/src/config/config.ts:1289` | 装配入口：创建 ToolRegistry、MCP servers、GeminiClient、SkillManager |
| `Config.getSettings()` | `config.ts` | 读取合并后配置，按 global → project → 环境变量 → CLI flag 优先级返回 |
| `loadDotEnv()` | `gemini-cli/packages/core/src/config/` | 加载 `.gemini/.env`，注入环境变量到进程 |
| `Config.getWorkspaceTrustPolicy()` | `config.ts` | 返回当前工作区信任等级，影响工具权限与 sandbox 策略 |
| `Storage.initialize()` | `gemini-cli/packages/core/src/config/storage.ts` | 计算全局/项目级持久化根目录，确保目录存在 |
