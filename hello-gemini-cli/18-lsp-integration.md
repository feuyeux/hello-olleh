...
| 能力 | 实现方式 | 质量 |
|------|---------|------|
| **文件读取** | `read_file` 工具 + 文本分析（`gemini-cli/packages/core/src/tools/read-file.ts:199`） | 全量但无语义 |
| **符号搜索** | `grep_search` / ripgrep（`gemini-cli/packages/core/src/tools/grep.ts:657`, `gemini-cli/packages/core/src/tools/ripGrep.ts:605`）| 模式匹配，无类型信息 |
| **代码库索引** | 无内置索引，依赖文件系统遍历 | 线性搜索 |
| **类型推断** | 依赖模型本身的语言理解能力 | 模型层面推断 |
| **诊断信息** | 不接入 LSP，需显式运行 lint/test/typecheck（`gemini-cli/packages/core/src/tools/shell.ts:942`） | 工具化补偿 |
...
| 函数/类型 | 文件 | 职责 |
|----------|------|------|
| `GrepTool` | `gemini-cli/packages/core/src/tools/grep.ts:657` | 代码搜索工具：通过正则在工作区文件中查找符号/文本 |
| `RipGrepTool` | `gemini-cli/packages/core/src/tools/ripGrep.ts:605` | ripgrep 优化路径 |
| `ReadFileTool` | `gemini-cli/packages/core/src/tools/read-file.ts:199` | 文件读取工具：支持按行范围读取代码上下文 |
| `ShellTool` | `gemini-cli/packages/core/src/tools/shell.ts:942` | 运行 lint/test/typecheck 补齐 diagnostics |
| `McpClientManager` | `gemini-cli/packages/core/src/tools/mcp-client-manager.ts:34` | MCP 接入桥接：第三方 LSP/代码分析 server 通过 MCP 接入 |
| `ToolRegistry` | `gemini-cli/packages/core/src/tools/tool-registry.ts:229` | LSP bridge 最终仍要注册成普通工具 |
...
| 能力 | Gemini 现状 | 横向对比 |
| --- | --- | --- |
| symbol lookup | `gemini-cli/packages/core/src/tools/grep.ts:657` | OpenCode/Claude 更强 |
| diagnostics | `gemini-cli/packages/core/src/tools/shell.ts:942` | Codex 类似 |
| MCP LSP | `gemini-cli/packages/core/src/tools/mcp-client-manager.ts:34` | 需要 ToolRegistry/Scheduler 接入 |
| IDE context | `gemini-cli/packages/core/src/ide/process-utils.ts:113` | 与 `21-bridge-system.md` 交叉 |
