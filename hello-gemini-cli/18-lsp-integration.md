...
| 函数/类型 | 文件 | 职责 |
|----------|------|------|
| `grep_search` tool | `gemini-cli/packages/core/src/tools/grep.ts` | 代码搜索工具：通过正则在工作区文件中查找符号/文本 |
| `read_file` tool | `gemini-cli/packages/core/src/tools/read-file.ts` | 文件读取工具：支持按行范围读取代码上下文 |
| `McpClientManager` | `gemini-cli/packages/core/src/mcp/` | MCP 接入桥接：第三方 LSP/代码分析 server 通过 MCP 接入 |
| `ContextManager.discoverContext()` | `gemini-cli/packages/core/src/services/contextManager.ts` | 按需发现文件上下文，补充代码理解能力 |
...
| 能力 | Gemini 现状 | 横向对比 |
| --- | --- | --- |
| symbol lookup | 非主线 | OpenCode/Claude 更强 |
| diagnostics | 通过 lint/test 工具补齐 | Codex 类似 |
| MCP LSP | 可扩展方向 | 需要 ToolRegistry/Scheduler 接入 |
| IDE context | 通过 IDE 集成补一部分 | 与 `21-bridge-system.md` 交叉 |

后续本章应写清楚“不支持实时 LSP”对重构、引用分析和诊断反馈的影响。
