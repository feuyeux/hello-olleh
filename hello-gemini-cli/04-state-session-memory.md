...
| 组件 | 代码路径 | 关键方法 | 职责 |
| :---| :---| :---| :---|
| **Storage** | `gemini-cli/packages/core/src/config/storage.ts` | `initialize()`, `getProjectTempDir()` | 计算全局/项目级存储根目录，提供会话与策略文件落点 |
| **ChatRecordingService** | `gemini-cli/packages/core/src/core/recordingContentGenerator.ts` | `recordMessage()` | 实时捕获交互并序列化落盘 |
| **UIStateContext** | `gemini-cli/packages/cli/src/ui/contexts/UIStateContext.tsx` | `useUIState()` | UI 状态容器，持有当前会话的渲染状态 |
| **MessageBus** | `gemini-cli/packages/core/src/confirmation-bus/message-bus.ts` | `publish()`, `subscribe()` | 事件总线广播，解耦工具执行层与 UI 层 |
| **SessionSelector** | `gemini-cli/packages/cli/src/utils/sessionUtils.ts` | `resolveSession()` | `--resume` 参数解析、会话查询与恢复入口 |
| **GitService** | `gemini-cli/packages/core/src/services/gitService.ts` | `getDiff()` | Checkpoint 时捕获 Git 状态 |
...
- **持久化根目录与路径计算**：`gemini-cli/packages/core/src/config/storage.ts`
- **UI 状态主入口**：`gemini-cli/packages/cli/src/ui/contexts/UIStateContext.tsx`
- **会话恢复核心逻辑**：`gemini-cli/packages/cli/src/gemini.tsx:553-585`
- **历史验证**：`gemini-cli/packages/core/src/core/geminiChat.ts:256-271`
...
`gemini-cli/packages/core/src/services/contextManager.ts` 把记忆分成三层：global memory、extension memory、project memory。这些内容来自 `gemini-cli/packages/core/src/utils/memoryDiscovery.ts`，后者扫描 `GEMINI.md` 及其别名文件，按来源分类、去重、拼接。
...
当前默认和预览模型的 token 上限统一为 `1_048_576`（见 `gemini-cli/packages/core/src/core/tokenLimits.ts`）。

`GeminiClient` 在真正流式发送前，会先调用 `tryCompressChat()`，由 `gemini-cli/packages/core/src/services/chatCompressionService.ts` 实现：
...
`gemini-cli/packages/core/src/services/loopDetectionService.ts` 提供完整的循环检测，接入点在 `gemini-cli/packages/core/src/core/client.ts`，包含三类检测：
...
`gemini-cli/packages/core/src/core/geminiChat.ts` 维护完整对话历史，包含：模型文本、`functionCall`、`functionResponse`、思考片段与补充内容。
...
| 主题 | 代码锚点 | 说明 |
| --- | --- | --- |
| 分层记忆加载 | `gemini-cli/packages/core/src/services/contextManager.ts` | 管 global / extension / project 三层记忆 |
| 记忆发现 | `gemini-cli/packages/core/src/utils/memoryDiscovery.ts` | 扫描、分类、去重 `GEMINI.md` |
| 系统提示词记忆注入 | `gemini-cli/packages/core/src/config/config.ts` | `getSystemInstructionMemory()` / `getSessionMemory()` |
| 会话历史维护 | `gemini-cli/packages/core/src/core/geminiChat.ts` | 保存完整历史并导出 curated history |
| 自动压缩 | `gemini-cli/packages/core/src/services/chatCompressionService.ts` | 压缩历史并处理大工具输出 |
| 发送前保护 | `gemini-cli/packages/core/src/core/client.ts` | 压缩、遮罩、溢出预警 |
| 循环检测 | `gemini-cli/packages/core/src/services/loopDetectionService.ts` | 工具重复、内容重复、LLM 辅助检测 |
| token 上限 | `gemini-cli/packages/core/src/core/tokenLimits.ts` | 当前默认模型窗口上限 |
...
从 `gemini-cli/packages/core/src/tools/memoryTool.ts` 可以看到，默认上下文文件名是：
...
`gemini-cli/packages/core/src/config/memory.ts` 定义了 `HierarchicalMemory`，包含三层：
...
核心逻辑在 `gemini-cli/packages/core/src/utils/memoryDiscovery.ts`，负责：
...
`gemini-cli/packages/core/src/config/config.ts` 里的分流逻辑：
...
`gemini-cli/packages/core/src/tools/memoryTool.ts` 的实现很直接：
...
`gemini-cli/packages/core/src/commands/memory.ts` 提供四类操作：
...
| 主题 | 代码锚点 | 说明 |
| --- | --- | --- |
| 记忆结构 | `gemini-cli/packages/core/src/config/memory.ts` | `HierarchicalMemory` 定义 |
| 记忆发现 | `gemini-cli/packages/core/src/utils/memoryDiscovery.ts` | 扫描、去重、分类 `GEMINI.md` |
| 记忆装载 | `gemini-cli/packages/core/src/services/contextManager.ts` | 管理 global / extension / project memory |
| 配置侧注入 | `gemini-cli/packages/core/src/config/config.ts` | system memory 与 session memory 分流 |
| 记忆写入工具 | `gemini-cli/packages/core/src/tools/memoryTool.ts` | `save_memory` 的真实实现 |
| Slash 命令包装 | `gemini-cli/packages/core/src/commands/memory.ts` | `/memory` 系列命令 |
...
| 函数 / 类型 | 文件 | 职责 |
| :---| :---| :---|
| `Storage.initialize()` | `gemini-cli/packages/core/src/config/storage.ts` | 计算全局/项目级存储根目录 |
| `ChatRecordingService.recordMessage()` | `gemini-cli/packages/core/src/core/recordingContentGenerator.ts` | 实时捕获交互并序列化落盘 |
| `SessionSelector.resolveSession()` | `gemini-cli/packages/cli/src/utils/sessionUtils.ts` | `--resume` 参数解析与会话恢复入口 |
| `GitService.getDiff()` | `gemini-cli/packages/core/src/services/gitService.ts` | Checkpoint 时捕获 Git 状态 |
| `MessageBus.publish()` / `subscribe()` | `gemini-cli/packages/core/src/confirmation-bus/message-bus.ts` | 事件总线广播，解耦工具执行与 UI |
| `Scheduler.schedule()` | `gemini-cli/packages/core/src/` | 只读工具并行、写操作顺序化 |
| `useGeminiStream.submitQuery()` | `gemini-cli/packages/cli/src/` | 有活跃流时拒绝新提交 |
| `ContextManager.refresh()` | `gemini-cli/packages/core/src/services/contextManager.ts` | 重新扫描并整理三层记忆 |
| `ContextManager.discoverContext()` | `gemini-cli/packages/core/src/services/contextManager.ts` | 按需路径发现子目录补充记忆（JIT） |
| `memoryDiscovery` | `gemini-cli/packages/core/src/utils/memoryDiscovery.ts` | 扫描 `GEMINI.md` 及别名，按来源分类、去重 |
| `Config.getSystemInstructionMemory()` | `gemini-cli/packages/core/src/config/config.ts` | 返回用于 system prompt 的 global memory |
| `Config.getSessionMemory()` | `gemini-cli/packages/core/src/config/config.ts` | 返回用于对话内注入的 extension/project memory |
| `Config.getTruncateToolOutputThreshold()` | `gemini-cli/packages/core/src/config/config.ts` | 根据剩余上下文动态收紧工具输出阈值 |
| `GeminiChat.getHistory(curated)` | `gemini-cli/packages/core/src/core/geminiChat.ts` | 返回过滤空 turn 后的对话历史 |
| `GeminiClient.tryCompressChat()` | `gemini-cli/packages/core/src/core/client.ts` | 发送前触发历史压缩 |
| `ChatCompressionService.compress()` | `gemini-cli/packages/core/src/services/chatCompressionService.ts` | 保留最近 30%，大工具输出落盘 |
| `ToolOutputMaskingService` | `gemini-cli/packages/core/src/services/` | 对历史中过大的工具输出二次瘦身 |
| `LoopDetectionService` | `gemini-cli/packages/core/src/services/loopDetectionService.ts` | 三类检测：工具重复（阈值5）、内容重复（阈值10）、LLM辅助（30轮后） |
| `tokenLimits` | `gemini-cli/packages/core/src/core/tokenLimits.ts` | 返回当前模型窗口上限（默认 1,048,576 tokens） |
| `HierarchicalMemory` | `gemini-cli/packages/core/src/config/memory.ts` | global / extension / project 三层记忆结构定义 |
| `setGeminiMdFilename()` | `gemini-cli/packages/core/src/tools/memoryTool.ts` | 改写默认记忆文件名 |
| `save_memory` 工具 | `gemini-cli/packages/core/src/tools/memoryTool.ts` | 读取 `GEMINI.md`，追加条目，展示 diff，写回磁盘 |
| `/memory` 命令 | `gemini-cli/packages/core/src/commands/memory.ts` | showMemory / addMemory / refreshMemory / listMemoryFiles |
