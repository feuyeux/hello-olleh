...
| 优先级 | 动作 | 完成标准 |
| --- | --- | --- |
| P0 | 四项目 `03/05/11/15/23/24/25` 都有源码锚点补强 | 每章至少 5 个 repo-root 路径 |
| P0 | Harness `14-20` 串成质量与合并闭环 | README、index、00-index 都可导航 |
| P1 | Codex/Gemini `11-21` 从摘要扩成生命周期链路 | 每章说明输入、处理、输出、横向差异 |
| P1 | OpenCode `26-38` 保持附录定位 | 不再复制 `03/13/19` 的总览结论 |
| P2 | 站点/PDF 渲染验证 | `cd pages && npm run build` 可跑通 |
