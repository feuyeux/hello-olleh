...
| 阶段 | 源码锚点 | 事件/状态 |
| --- | --- | --- |
| 订阅入口 | `opencode/packages/opencode/src/server/routes/event.ts:19` | `/event` 暴露 `event.subscribe` |
| SSE stream | `opencode/packages/opencode/src/server/routes/event.ts:35` | 通过 `streamSSE()` 持续输出事件 |
| Bus 全局转发 | `opencode/packages/opencode/src/bus/index.ts:59` | `GlobalBus.emit("event", ...)` 汇总所有事件 |
| session busy/idle | `opencode/packages/opencode/src/session/status.ts:73` | `session.status` 事件通知运行态 |
| message update | `opencode/packages/opencode/src/session/index.ts:700` | message 更新进入 Bus |
| part update | `opencode/packages/opencode/src/session/index.ts:770` | `message.part.updated` 是 UI/CLI 的主要增量信号 |
| part delta | `opencode/packages/opencode/src/session/index.ts:787` | text/reasoning delta 用轻量事件推送 |
| CLI 消费 | `opencode/packages/opencode/src/cli/cmd/run.ts:447` | `run` 命令遍历 SDK event stream |
