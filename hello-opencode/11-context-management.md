...
这里也先把边界说清楚：本篇只关心 Skill 怎样进入 system prompt 和模型上下文；Skill 自己的发现、授权、加载链路放在 [14-skill-system.md](./14-skill-system.md)，而它作为扩展入口怎样和 Plugin/MCP/Command 并列，则放到当前承载“扩展面”主题的 [23-bridge-system.md](./23-bridge-system.md)。
...
- [1. 上下文源头不是一层，而是多层叠加](#1-上下文源头不是一层而是多层叠加)
- [2. 输入侧：用户输入会先被改写成可持久化上下文](#2-输入侧用户输入会先被改写成可持久化上下文)
- [3. 指令系统不是只有根目录 `AGENTS.md`](#3-指令系统不是只有根目录-agentsmd)
- [4. system prompt 的真实组装顺序](#4-system-prompt-的真实组装顺序)
- [5. 运行时提醒会在进入模型前再改写一次历史](#5-运行时提醒会在进入模型前再改写一次历史)
- [6. Durable history 投影成模型消息时还会再“翻译一遍”](#6-durable-history-投影成模型消息时还会再翻译一遍)
- [7. 工具集合也是上下文的一部分](#7-工具集合也是上下文的一部分)
- [8. 把 B02 压成一句代码级结论](#8-把-b02-压成一句代码级结论)
...
| `createUserMessage()` | `session/prompt.ts` | 决定 user message 的解释者（规则/assistant/system）|
| `system.ts` 层叠器 | `session/system.ts` | 组装 system prompt：provider + environment + instructions + skill 四层 |
| `AGENTS.md` 加载器 | `project/` | 从当前目录向上查找并层叠所有 `AGENTS.md` 文件 |
| `DurableHistory.filterCompacted()` | `session/index.ts` | 过滤已 compact 的历史，仅向模型投影有效消息 |
| `compactMessages()` | `session/compact.ts` | 历史压缩：将旧历史 summary 化，生成 compact session |
