...
| 函数/类型 | 文件 | 职责 |
|----------|------|------|
| `prompt()` | `session/prompt.ts:1029-1336` | prompt 阶段主入口：编译 PromptInput → durable message |
| `PromptInput` (type) | `session/prompt.ts` | prompt 阶段中间表示：收束用户输入的所有 part 来源 |
| `resolvePromptParts()` | `session/prompt.ts` | 将模板引用（file/agent/skill）预编译成具体 part |
| `createUserMessage()` | `session/prompt.ts` | 决定 user message 的解释角色（规则/assistant/system） |
| `Plugin.trigger('chat.message')` | `plugin/index.ts` | 在 user message 落库前允许 plugin 改写内容 |
| `Session.updateMessage()` | `session/index.ts` | 将编译后的消息写入 durable history |
