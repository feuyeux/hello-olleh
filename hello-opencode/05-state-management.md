...
- [1. Durable State 写入口](#1-durable-state-写入口)
- [2. SQLite 表结构](#2-sqlite-表结构)
- [3. `Database.effect()` 保证"先写库再发事件"](#3-databaseeffect-保证先写库再发事件)
- [4. Message 与 Part 的关系](#4-message-与-part-的关系)
- [5. 三条消费链](#5-三条消费链)
- [6. MessageV2 关键函数](#6-messagev2-关键函数)
- [7. 并发占位机制](#7-并发占位机制)
- [8. SessionStatus 与 Durable State 的区别](#8-sessionstatus-与-durable-state-的区别)
- [9. Snapshot 与 Diff](#9-snapshot-与-diff)
- [10. 关键函数清单](#10-关键函数清单)
...
- **`computeDiff()` 计算开销**：`SessionSummary.computeDiff()` 每次都重新计算 session 差分，在长会话场景下可能产生性能热点，缺少增量计算或缓存策略。
