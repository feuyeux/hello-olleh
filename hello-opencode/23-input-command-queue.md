...
OpenCode 的消息系统核心是 `MessageV2`（`opencode/packages/opencode/src/session/message-v2.ts`），它将所有用户输入编译成 `MessageV2.Info` + `MessageV2.Part[]` 的结构，持久化到 SQLite。
...
`opencode/packages/opencode/src/session/prompt.ts:95-160` 定义了输入格式：
...
`opencode/packages/opencode/src/command/index.ts:33-51`：
...
`opencode/packages/opencode/src/command/index.ts:75-158` 整合三类命令：
...
| 结构 | 位置 | 用途 |
| :------| :------| :------|
| `PromptInput` | `opencode/packages/opencode/src/session/prompt.ts:95-160` | HTTP API 输入格式 |
| `MessageV2.Info` | `opencode/packages/opencode/src/session/message-v2.ts:351-375` | User/Assistant 消息元信息 |
| `MessageV2.Part` | `opencode/packages/opencode/src/session/message-v2.ts:377-395` | 消息内容片段（text/file/tool等） |
| `TextPart` | `opencode/packages/opencode/src/session/message-v2.ts:104-119` | 纯文本片段 |
| `FilePart` | `opencode/packages/opencode/src/session/message-v2.ts:175-184` | 文件/资源片段 |
| `ToolPart` | `opencode/packages/opencode/src/session/message-v2.ts:335-344` | 工具调用记录 |
| `Command.Info` | `opencode/packages/opencode/src/command/index.ts:33-51` | 命令定义 |
| `CommandInput` | `opencode/packages/opencode/src/session/prompt.ts:1790-1811` | 命令执行输入 |
...
| --- | --- | --- |
| prompt | `opencode/packages/opencode/src/server/routes/session.ts` -> `SessionPrompt.prompt()` | durable user message 编译 |
...
| TUI event | `opencode/packages/opencode/src/server/routes/tui.ts` / Bus | 多表面输入统一投影 |
