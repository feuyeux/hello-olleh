...
- [1. 状态管理概述](#1-状态管理概述)
- [2. AppState 定义](#2-appstate-定义)
- [3. Store 实现](#3-store-实现)
- [4. AppStateStore](#4-appstatestore)
- [5. 状态选择器](#5-状态选择器)
- [6. 状态变更处理](#6-状态变更处理)
- [7. 状态更新模式](#7-状态更新模式)
- [8. React 集成](#8-react-集成)
- [9. 持久化](#9-持久化)
- [10. 状态调试](#10-状态调试)
...
| 函数/类型 | 文件 | 职责 |
|----------|------|------|
| `AppState` | `src/AppState.ts` | 全局状态类型定义：消息、连接、工具调用状态等 |
| `createAppStateStore()` | `src/AppState.ts` | 工厂函数：创建带选择器的 observable store |
| `AppStateStore.update()` | `src/AppState.ts` | 状态变更入口，触发订阅者通知 |
| `selectMessages()` | — | 消息列表选择器 |
| `selectToolUseContext()` | — | 工具执行上下文选择器 |
...
- **单向数据流**：所有状态通过 `AppStateStore.update()` 变更，UI 只读不写，数据流向单一，竞态条件大幅减少。
- **选择器模式**：组件只订阅自己关心的状态切片，避免无关状态变更触发不必要的重渲染，Ink TUI 性能受益明显。
- **状态类型完整**：`AppState` 明确定义了所有运行时状态字段，没有隐式 mutable 全局变量，状态追踪成本低。

## 跨工具横向对比

Claude Code 的 Transcript 方案（JSONL 追加写）是最简单的实现，代价是无事务保证、无原子写回、无并发控制。完整的四工具对比见 **[hello-opencode/39-durable-state-comparison.md](../hello-opencode/39-durable-state-comparison.md)**。
