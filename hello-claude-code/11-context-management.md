...
- **梯度上下文体系**：系统始终携带 system prompt + 最近消息 + 工具声明，仅按需附加记忆/项目脚手架/用户上下文，token 预算随会话状态动态调配。
- **Compact prompt 自愈**：上下文溢出时主动触发 compact，生成 summary 而不是截断，保留语义连贯性而非丢弃旧 turn。
- **Prompt cache 友好设计**：session 级 cache_control 标签让 Anthropic API 在多轮请求间复用 prompt 前缀，降低 API 调用延迟和 token 成本。

**风险与改进点**

- **`compact` 触发语义不透明**：用户不知道当会话在"正常运行"还是"正在压缩上下文"，compact 的语义损失率也无法量化。
- **记忆（Memory）系统与上下文管理松耦合**：`Memory.md` 注入与 transcript 恢复是两条独立路径，恢复 session 时记忆文件变更不会自动同步到恢复后的对话上下文。
