...
| --- | --- | --- | --- |
| 模型请求失败恢复 | 429/5xx、stream fallback、context overflow、retry | `claude-code/src/services/api/withRetry.ts:170`, `claude-code/src/services/api/claude.ts:1539`, `claude-code/src/services/api/claude.ts:1779` | 用户请求可继续或给出明确 API 错误 |
| 请求参数安全 | thinking/cache/beta/fast mode 等请求字段 | `claude-code/src/services/api/claude.ts:834`, `claude-code/src/services/api/claude.ts:1737` | 不向 provider 发送互斥或危险参数组合 |
| 工具使用授权 | Bash/Edit/MCP 等本地动作是否允许 | `claude-code/src/hooks/useCanUseTool.tsx:28`, `claude-code/src/services/tools/toolExecution.ts:30` | 高风险动作被 allow/deny/ask 正确分流 |
| 企业/项目策略 | settings、permission rule、MCP allow/deny | `claude-code/src/utils/settings/validation.ts:5`, `claude-code/src/utils/settings/types.ts:12` | 本地偏好不能越过托管策略 |
| UI 确认与状态 | 用户确认队列、permission context | `claude-code/src/screens/REPL.tsx:2386` | 等待确认时不误执行，也不伪装成 provider retry |
