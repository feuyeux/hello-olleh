...
Codex 使用 Rust 的 `tracing` crate。TUI 主循环和工具任务会在异常路径上写 `warn!`/`debug!`/`error!`，例如 app-server shutdown、terminal input stream closed、用户 shell command failed 等（`codex/codex-rs/tui/src/app.rs:686`, `codex/codex-rs/tui/src/app.rs:1025`, `codex/codex-rs/core/src/tasks/user_shell.rs:274`）。
...
`EventMsg` 是更重要的可观测性骨架：协议层枚举事件类型（`codex/codex-rs/protocol/src/protocol.rs:1515`），regular turn 直接发出 `TurnStarted`（`codex/codex-rs/core/src/tasks/regular.rs:47`），工具执行由 `ToolEmitter` 发出 begin/end/failure 事件（`codex/codex-rs/core/src/tools/events.rs:151`）。
...
| 层 | 源码锚点 | 触发时机 |
| --- | --- |
| TUI app event | `codex/codex-rs/tui/src/app_event.rs:105` | UI 组件、后台请求、thread routing 之间的内部消息 |
| protocol event | `codex/codex-rs/protocol/src/protocol.rs:1515` | core 对 TUI/SDK 暴露的稳定事件 |
| turn lifecycle | `codex/codex-rs/core/src/tasks/regular.rs:47` | 回合开始 |
| tool lifecycle | `codex/codex-rs/core/src/tools/events.rs:74` | exec/apply_patch begin/end/failure |
| timing state | `codex/codex-rs/core/src/turn_timing.rs:43` | TTFT/TTFM 统计 |
...
| 链路 | 源码锚点 | 说明 |
| --- | --- | --- |
| 事件流 | `codex/codex-rs/protocol/src/protocol.rs:1515` | TUI/SDK 可消费的 turn/tool/approval/error 事件 |
| 工具事件 | `codex/codex-rs/core/src/tools/events.rs:151` | 将 exec/apply_patch begin/end/failure 映射成 protocol events |
| 反馈诊断 | `codex/codex-rs/feedback/src/feedback_diagnostics.rs:14` | 收集 connectivity diagnostics 供用户反馈附带 |
| rollout reconstruction | `codex/codex-rs/core/src/session/rollout_reconstruction_tests.rs:1289` | 测试覆盖历史事件重建与 resume 边界 |
| UI 路由 | `codex/codex-rs/tui/src/app.rs:993` | TUI 主循环集中处理 app event、thread event、terminal event、app-server event |
...
| 主题 | 代码锚点 | 说明 |
| --- | --- | --- |
| 协议事件枚举 | `codex/codex-rs/protocol/src/protocol.rs:1515` | UI/SDK 消费的主事件集合 |
| TUI 内部事件 | `codex/codex-rs/tui/src/app_event.rs:105` | UI 组件与 app loop 的内部消息总线 |
| 主循环 select | `codex/codex-rs/tui/src/app.rs:993` | 同时监听 app event、thread event、terminal event 和 app-server event |
| 工具事件映射 | `codex/codex-rs/core/src/tools/events.rs:151` | exec/apply_patch 事件发射 |
| turn timing | `codex/codex-rs/core/src/turn_timing.rs:43` | TTFT/TTFM 统计状态 |
| feedback diagnostics | `codex/codex-rs/feedback/src/feedback_diagnostics.rs:14` | 反馈附件诊断结构 |
