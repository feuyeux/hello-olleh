...
| 能力 | 实现方式 | 位置 |
|------|---------|------|
| **文件读取** | Shell / apply_patch / dynamic tools | `codex/codex-rs/tools/src/tool_spec.rs:22` |
| **符号搜索** | `grep` / `rg`（ripgrep） | `codex/codex-rs/shell-command/src/command_safety/is_safe_command.rs:114` |
| **结构分析** | Shell 工具：`tree`, `find` | shell command |
| **类型信息** | 编译器输出（`cargo check`, `tsc`） | shell command |
| **诊断** | 编译器/Lint/测试/feedback diagnostics | `codex/codex-rs/feedback/src/feedback_diagnostics.rs:14` |
...
实际源码中，`apply_patch` 先解析并验证补丁，再通过 core handler 进入工具事件与权限链路（`codex/codex-rs/apply-patch/src/lib.rs:183`, `codex/codex-rs/core/src/tools/handlers/apply_patch.rs:377`）。这提供的是“精确补丁 + 失败诊断”，不是 LSP 语义编辑。
...
这种方式虽然比 LSP 慢（每次需要完整编译），但对于 Rust 等强类型语言，编译器提供的诊断信息往往比 LSP hover 更权威。注意它仍然受工具治理影响：`cargo check` 在安全命令分类里不是自动安全命令（`codex/codex-rs/shell-command/src/command_safety/is_safe_command.rs:426`），因此可能进入 approval/sandbox 流程。
...
| 函数/类型 | 文件 | 职责 |
|----------|------|------|
| `ToolSpec` | `codex/codex-rs/tools/src/tool_spec.rs:22` | Codex 工具集合抽象，不包含原生 LSP tool |
| `create_local_shell_tool()` | `codex/codex-rs/tools/src/tool_spec.rs:83` | 暴露 shell 能力，间接调用编译器/搜索工具 |
| `is_known_safe_command()` 的 `rg` 分支 | `codex/codex-rs/shell-command/src/command_safety/is_safe_command.rs:114` | 将常见只读搜索命令纳入安全判断 |
| `cargo_check_is_not_safe()` | `codex/codex-rs/shell-command/src/command_safety/is_safe_command.rs:426` | 说明编译器验证不等于免审批 |
| `DynamicToolSpec` | `codex/codex-rs/protocol/src/dynamic_tools.rs:10` | 外部 bridge 可通过 dynamic tools 暴露 LSP 类能力 |
| `validate_dynamic_tools()` | `codex/codex-rs/app-server/src/codex_message_processor.rs:9145` | app-server 校验外部动态工具 schema |
