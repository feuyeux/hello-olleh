...
| 表面 | 主要入口 | Runtime 边界 |
| --- | --- | --- |
| CLI/TUI | `codex/codex-rs/cli`, `codex/codex-rs/tui` | 直接进入 Rust workspace |
| app-server | `codex/codex-rs/app-server` | 把 session/thread 能力暴露给外部宿主 |
| TypeScript SDK | `codex/sdk` | 消费 Codex JSON event protocol，不拥有 agent loop |
| WebSocket/HTTPS | `codex/codex-rs/core/src/client.rs` | 采样请求可走 WebSocket 或 HTTPS，Prompt shape 由 core 侧提供 |
...
| 源码位置 | 说明 | 横向意义 |
| --- | --- | --- |
| `codex/codex-rs/tui/src/lib.rs:372` | TUI 选择 embedded/remote app-server | 对应 OpenCode server-first 入口 |
| `codex/codex-rs/app-server/src/lib.rs:355` | app-server transport 启动入口 | 暴露 JSON-RPC 控制面 |
| `codex/codex-rs/app-server/src/transport/mod.rs:74` | `stdio://` / `ws://` listen URL 解析 | 说明外部宿主协议边界 |
| `codex/codex-rs/app-server-client/src/lib.rs:470` | in-process app-server client 启动 | 对应本地 TUI 嵌入模式 |
| `codex/codex-rs/app-server-client/src/remote.rs:139` | remote WebSocket client connect | 对应远程宿主模式 |
| `codex/codex-rs/core/src/client.rs:1485` | 模型 Responses WebSocket/HTTP streaming | 与 app-server transport 分层 |
